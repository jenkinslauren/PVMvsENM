...
Cleaning *Fraxinus pennsylvanica*.
...
Working with imprecise coordinates:
```{r}
...
plot(littleRange_FraxPenn, col = alpha('blue', 0.4), border = 'blue', 
...
Thinning with the above method gave 14,149 observations, while the longer method gave 14,117 observations.
finalThinned with longer method gave 558 observations and the shorter method gave the same!

After removing 18,214 imprecise coordinates, our thinned data had 515 observations and before removing cell duplicates, there were 14,119.

Before eliminating cell duplicates (14,119 observations):
```{r}
...
if (file.exists(bufferFileName)) {
  ...
  rangeMap <- st_transform(st_as_sf(x = littleRange_FraxPenn), getCRS('albersNA'))
...
Map again with buffer and ranges, focused on Florida to see buffer.
```{r}
ggplot(data = world) +
  theme_bw() +
  geom_sf(fill = "white") +
...
```{r}
save.image('./03 - Modeling Workspace - Frax_Penn Cleaning')
