...
load('./03 - Modeling Workspace - Frax_Dipe Cleaning')
...
dir.create('./Models/Frax_Dipe_Maxent',
    recursive=TRUE, showWarnings=FALSE)

# model species
envModel <- maxnet(p = presBg, data = trainData)
save(envModel, file='./Models/Frax_Dipe_Maxent/Model.Rdata', compress=TRUE)
```
...
envMap <- predict(
  climate[[predictors]],
  envModel,
  filename='./Models/Frax_Dipe_Maxent/maxentPredictionBeyer0KYBP', 
  clamp = F, 
  format='GTiff', 
  overwrite = TRUE, 
  type='cloglog')
...
```{r}
plot(littleRange_FraxDipe, border = 'blue', main = "Maxent Output, Frax_dipe")
plot(envMap, add = TRUE)
plot(littleRange_FraxDipe, border = 'blue', add = TRUE)
map("state", add = TRUE)
map("world", add = TRUE)
...
```{r}
save.image('./05 - Modeling Workspace - Frax_dipe Model Output')
load('./05 - Modeling Workspace - Frax_dipe Model Output')
```
