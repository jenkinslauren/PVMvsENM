---
title: "Fraxinus caroliniana cleaning"
...
speciesAb <- "Frax_Caro"
range <- littleRange_FraxCaro
title <- expression(paste(italic("Fraxinus caroliniana"), " occurrences"))
```
...
```
43 records removed due to imprecise coordinates.
...
print(paste0("Number of occurrences outside buffer = ", 
             length(which(speciesSf_filtered$within_buffer == 0))))
```
The 2 occurrences outside the buffer were from Cuba, where the Little range didn't extend.
