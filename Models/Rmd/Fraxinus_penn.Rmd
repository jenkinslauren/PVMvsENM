...
222 = full species name, all lowercase, _ in between (fraxinus_pennsylvanica)
333 = abreviated species name (ex. FraxPenn)
444 = abreviated with _ in between 
...
pc <- 5
fileName <- paste0('./data_and_analyses/env_data/Beyer/envDataClipped_0kybp_pca', pc, '.tif')
if (file.exists(fileName)) {
  ...
  envPresences <- stack(envDataPca[[1:pc]])
  names(envPresences) <- paste0('pca', 1:pc)
...
plot(recordsSp, pch = 16, cex = 0.5, col = "red", 
     main = substitute(paste(italic('Fraxinus pennsylvanica '), 'occurrences (BIEN) thinned')))
if (exists("water")) {
