---
title: "Fraxinus berlandieriana cleaning"
...
speciesAb <- "Frax_Berl"
range <- littleRange_FraxBerl
projection(range) <- getCRS('wgs84')
title <- expression(paste(italic("Fraxinus berlandieriana"), " occurrences"))
```
...
Cleaning *Fraxinus berlandieriana*.
...
plot(occsSp, pch = 16, cex = 0.6, col = 'red', 
...
if (file.exists(bufferFileName)) {
  ...
  rangeMap <- st_transform(st_as_sf(x = range), crs = getCRS('albersNA'))
...
ggplot(data = world) +
  ...
  coord_sf(xlim = c(min(thinned$longitude) - 5, max(thinned$longitude) + 5), 
           ylim = c(min(thinned$latitude) - 5, max(thinned$latitude) + 2), expand = TRUE) +
  xlab("Longitude") + 
  ylab("Latitude") +
  ggtitle(title, 
...
ggplot(data = world) +
  ...
  coord_sf(xlim = c(min(finalThinned$longitude) - 5, 
           ...
           ylim = c(min(finalThinned$latitude) - 5, 
                    max(finalThinned$latitude) + 2), 
           expand = TRUE) +
  xlab("Longitude") +
  ylab("Latitude") +
  ggtitle(title, subtitle = paste0("(Cleaned, thinned, and duplicates eliminated, with buffer = ",
