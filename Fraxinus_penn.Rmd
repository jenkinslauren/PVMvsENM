---
title: "Fraxinus pennsylvanica model"
...
```{r, include = FALSE}
fileName <- './data_and_analyses/env_data/Beyer/envDataClipped.tif'
...
dir.create('./Models/Frax_Penn_Maxent',
    recursive=TRUE, showWarnings=FALSE)

# model species
envModel <- maxnet(p = presBg, data = trainData)
save(envModel, file='./Models/Frax_Penn_Maxent/Model.Rdata', compress=TRUE)
```
...
envMap <- predict(
  climate[[predictors]],
  envModel,
  filename='./Models/Frax_Penn_Maxent/maxentPredictionBeyer0KYBP', 
  clamp = F, 
  format='GTiff', 
  overwrite = TRUE, 
  type='cloglog')
...
```
```{r}
plot(littleRange_FraxPenn, border = 'blue', main = "Maxent Output, Frax_penn")
...
```
```{r}
save.image('./05 - Modeling Workspace - Frax_penn Model Output')
load('./05 - Modeling Workspace - Frax_penn Model Output')
```
